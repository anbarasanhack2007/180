...
final userSkillsMapProvider = FutureProvider<Map<String, Map<String, dynamic>>>((ref) async {
...
class SkillsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final categories = ['Foundation', 'Programming', 'Offensive', 'Defensive', 'Career'];
    final categoryIcons = {
          ...
          data: (skills) => userSkillsAsync.when(
            data: (userSkills) => ListView(
              ...
              children: categories.map((cat) {
                final catSkills = skills.where((s) => s['category'] == cat).toList();
                ...
                return Column(
                  ...
                  children: [
                    ...
                    ...catSkills.map((skill) {
                      ...
                      final progress = (us?['progress'] as int? ?? 0).toDouble() / 100;
                      return Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            if ((skill['description'] as String?)?.isNotEmpty ?? false) ...[
                              const SizedBox(height: 4),
                              Text(skill['description'] as String,
                                  style: const TextStyle(color: AppColors.textHint, fontSize: 12), maxLines: 2, overflow: TextOverflow.ellipsis),
                            ],
                            const SizedBox(height: 8),
                            Row(children: [
                              Expanded(child: LinearProgressIndicator(value: progress, backgroundColor: AppColors.progressBg, valueColor: AlwaysStoppedAnimation<Color>(color), minHeight: 4)),
                              const SizedBox(width: 8),
                              Text('${(progress * 100).toInt()}%', style: TextStyle(color: color, fontSize: 12, fontWeight: FontWeight.w600)),
                            ]),
                            const SizedBox(height: 8),
                            Wrap(spacing: 6, children: ['not_started', 'learning', 'practicing', 'completed'].map((s) {
                              ...
                              return GestureDetector(
                                onTap: () async {
                                  ...
                                  await SupabaseService.userSkills.upsert({
                                    ...
                                    'skill_id': skill['id'],
                                    'status': s,
                                    'progress': s == 'completed' ? 100 : (s == 'practicing' ? 75 : (s == 'learning' ? 25 : 0)),
                                ...
                                child: Container(
                                  ...
                                  child: Text(s.replaceAll('_', ' '),
                                      style: TextStyle(color: selected ? color : AppColors.textHint, fontSize: 10, fontWeight: FontWeight.w600)),
                                ),
                              );
                            }).toList()),
                          ],
  ...
  Widget _statusBadge(String status, Color color) {
    final labels = {'not_started': 'NOT STARTED', 'learning': 'LEARNING', 'practicing': 'PRACTICING', 'completed': 'DONE'};
    final colors = {
    ...
    return Container(
      ...
      child: Text(labels[status] ?? status.toUpperCase(), style: TextStyle(color: c, fontSize: 9, fontWeight: FontWeight.w700, letterSpacing: 0.5)),
    );
  }
}
